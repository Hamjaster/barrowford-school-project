...
export const getTeacherProfile = async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    // 2️⃣ Get class name from `classes` table
    let className = null;
    if (teacher.class_id) {
    ...
    // 3️⃣ Fetch students assigned to this teacher (same logic as getAssignedStudents)
    const { data: students, error: studentError } = await supabase
      .from('students')
      .select('id, first_name, last_name, email, year_group_id, class_id, created_at')
      .eq('year_group_id', teacher.year_group_id)
    ...
    // 4️⃣ Build and send final response
    res.json({
      success: true,
      teacher: {
        ...teacher,
        class_name: className, // include readable class name
      },
      students: students || [],
    });
