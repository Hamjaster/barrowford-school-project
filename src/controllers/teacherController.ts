...
export const getAssignedStudents = async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    // ✅ Safely map class name (array fix)
    const studentsWithClassName = students?.map((student: any) => ({
      ...student,
    ...
    res.json({ success: true, data: studentsWithClassName });
  } catch (err: any) {
...
export const updateStudentProfilePhoto = async (req: any, res: any) => {
  try {
    const { studentId, profilePhotoUrl } = req.body;
console.log("controllerstudentid",studentId)
console.log("controllerprofilephotourl",profilePhotoUrl)
    if (!studentId || !profilePhotoUrl) {
      return res.status(400).json({ error: "studentId and profilePhotoUrl are required" });
    }
